...
void TimeUpdateTask(void *argument)
{
    ...
    for (;;) {
        ...
        osDelay(OV_TIME_REFRESH_PERIOD_MS);
    }
}
...
void refresh_current_time_on_scr1(void)
{
    ...
    time_update_data_t *p_data;
    const char* weekdays[] = {"", "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};

    HAL_RTC_GetTime(&hrtc, &nowtime, RTC_FORMAT_BIN);
    HAL_RTC_GetDate(&hrtc, &nowdate, RTC_FORMAT_BIN);

    p_data = (time_update_data_t *)pvPortMalloc(sizeof(time_update_data_t));
    if (p_data == NULL) {
        osDelay(OV_TIME_REFRESH_PERIOD_MS);
        return;
    }
    ...
    if (nowdate.WeekDay >= 1 && nowdate.WeekDay <= 7) {
        strncpy(p_data->week_str, weekdays[nowdate.WeekDay], sizeof(p_data->week_str) - 1U);
        p_data->week_str[sizeof(p_data->week_str) - 1U] = '\0';
    } else {
